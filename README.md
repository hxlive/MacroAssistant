...
当前版本完成了核心架构整改：屏幕定位统一由 `screen_locator.py` 管理，宏步骤状态和配置交互统一由 `step_controller.py` 管理；同时新增拖动、屏幕颜色判断和相对识别区域，并增强了运行停止、多屏坐标、搜索范围预览、OCR/图像缓存、日志诊断和打包环境兼容性。
...
- **🛡️ 即时安全中断**：支持默认全局停止热键 `Ctrl+F2`、PyAutoGUI failsafe，以及 RUN 子进程清理。
- **⌨️ 快速参数录入**：移动、点击和拖动坐标支持移动鼠标后按 `F8` 自动取点；颜色判断还会同时记录当前像素颜色。按键动作支持直接按下单键或组合键录入，并主动避让运行/停止快捷键。
- **🖱️ 基础鼠标动作**：支持点击、绝对/相对移动、滚动和按住拖动；动作中的持续时间、间隔及超时统一使用毫秒。
- **🔍 图像与文字识别**：支持模板图像查找、OCR 文字定位、识别结果保存到剪贴板或变量，并可选择全屏、固定区域或相对上次识别位置进行搜索。
- **🎨 屏幕颜色判断**：可按指定像素颜色和 RGB 容差执行 IF 分支，支持匹配与不匹配两种条件。
...
- **📊 被动运行状态条**：运行时显示当前循环和停止热键，可放在任务栏上沿或任务栏内部；窗口不获取焦点并穿透鼠标，避免干扰宏操作。
- **🎯 图标一致性**：Python 与 EXE 启动时会在主窗口创建前加载应用图标，主窗口、运行状态条及打包程序使用一致的 MacroMate 图标。
- **💾 配置保护**：应用设置使用专用 `.mmcfg` 文件、原子写入和跨进程锁；AI API Key 通过 Windows DPAPI 加密保存。
...
- 默认停止热键：**Ctrl+F2**
- 主窗口运行按钮可启动或停止宏
- 运行悬浮条只显示进度和停止热键，不接收鼠标点击，避免与宏争夺鼠标控制
...
动作编号以当前 v1.8.6 界面顺序为准。所有动作中由用户填写的持续时间、间隔和超时均使用毫秒；AI 大模型配置中的网络超时仍使用秒。
...
| 08 | 按住并拖动 | 从起点按住指定鼠标按键拖动到终点，拖动持续时长使用毫秒。 |
| 09 | 激活窗口（按标题） | 按标题查找并激活窗口，可配置失败处理。 |
| 10 | 备注 | 添加说明；以 LABEL: 或 标签: 开头时可定义跳转标签。 |
| 11 | 查找图像 | 在全屏、固定区域或相对上次识别位置查找模板，并定位到目标中心。 |
| 12 | 查找文本（OCR） | 定位文字，可保存识别原文并用正则提取字段。 |
| 13 | IF 找到图像 | 找到模板时执行 IF 块。 |
| 14 | IF 找到文本 | 找到目标文字时执行 IF 块。 |
| 15 | IF 颜色匹配 | 根据指定坐标的像素颜色、RGB 容差和匹配方式执行 IF 块。 |
| 16 | IF 变量比较 | 按文本或数值条件执行分支。 |
| 17 | ELSE | IF 条件不成立时执行。 |
| 18 | END_IF | 结束 IF/ELSE 块。 |
| 19 | 循环开始 | 固定次数循环，或直到找到图像/文字。 |
| 20 | 结束循环 | 结束普通循环。 |
| 21 | 设置变量 | 创建或更新运行变量。 |
| 22 | 变量计算 | 使用安全表达式进行基础数值计算。 |
| 23 | 正则提取变量 | 从文本中提取匹配内容并保存。 |
| 24 | 人工输入 | 暂停宏并弹出输入框，将结果保存为变量。 |
| 25 | 跳转到标签 | 跳转到备注步骤定义的标签。 |
| 26 | 条件跳转 | 条件成立时跳转到指定标签。 |
| 27 | 读取文本文件到变量 | 从允许的路径读取文本。 |
| 28 | 写入文本文件 | 覆盖或追加文本到允许的路径。 |
| 29 | 批量处理文本行 | 从文本或文件逐行读取数据。 |
| 30 | 结束批量处理 | 结束逐行处理块。 |
| 31 | 执行命令/脚本 | 在明确授权后执行外部命令或脚本。 |
| 32 | AI 自然语言指令 | 使用 VLM 分析截图并定位目标坐标。 |
...
图像、OCR、对应的 IF 条件和视觉循环条件支持三种搜索范围：

- **全屏**：在整个虚拟桌面搜索。
- **固定区域**：使用框选或填写的绝对坐标搜索。
- **相对上次识别位置**：以上一次成功的图像/OCR 识别结果中心为锚点，通过 X/Y 偏移和宽高确定本次区域。
...
点击“框选”可重新选择固定区域；点击“预览”会直接在当前桌面显示红色边框，并分别在左上角和右下角显示坐标，不会遮盖桌面内容。

相对区域只使用图像/OCR 的成功识别位置，不会被普通鼠标移动覆盖。测试相对区域时，需要先测试前一个识别步骤以建立测试锚点。固定区域和相对区域都属于明确约束，增强匹配失败后不会越过指定范围回退到全屏搜索。
...
- API Key 使用 Windows DPAPI 加密后保存在本地 **macro_settings.mmcfg**，仅当前 Windows 用户可以解密；不要分享该文件。
...
应用设置保存在专用扩展名的 **macro_settings.mmcfg** 中，宏文件仍使用 `.json`。这样可以在保存宏时更直观地区分两类文件，并降低误覆盖应用配置的概率。

- 确认属于 MacroMate 的旧版 `macro_settings.json` 会自动迁移；无法确认的 JSON 文件不会被当作配置迁移。
- 配置与宏文件均采用原子写入，写入中断时不会直接破坏原文件。
- 配置写入使用跨进程锁，减少多个 MacroMate 实例同时保存导致的覆盖。
- 保存宏时会拒绝使用当前应用配置文件作为目标。
- 读取并重新保存旧宏时，旧时间字段会按原单位换算，避免运行速度发生变化；拖动动作直接使用新的毫秒字段。
